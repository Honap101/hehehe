...
if PDF_AVAILABLE:
    st.success("📄 PDF report generation is available!")
else:
...
st.markdown("*Developed by Team HI-4requency for BPI DATA Wave 2025*") base64.b64encode(data).decode()
...
def create_fhi_score_banner(fhi_score: float) -> Drawing:
    ...
    # Background rectangle with gradient effect (simulated with two rectangles)
    d.add(Rect(0, 0, 500, 80, fillColor=colors.white, strokeColor=colors.HexColor("#e5e7eb"), strokeWidth=1))
    
    # Gradient bar at top (simulated)
...
def create_component_chart(components: dict) -> Drawing:
...
def generate_fynstra_pdf(fhi_score: float, components: dict, user_inputs: dict) -> bytes:
    ...
    try:
        styles, FYNSTRA_RED, FYNSTRA_ORANGE, FYNSTRA_GOLD, LIGHT_GRAY = create_pdf_styles()
        ...
        # FHI Score Banner
        story.append(create_fhi_score_banner(fhi_score))
        story.append(Spacer(1, 20))
        ...
        story.append(create_data_table(profile_data))
        story.append(Spacer(1, 20))
        ...
        # Component chart
        story.append(create_component_chart(components))
        story.append(Spacer(1, 15))
        ...
        story.append(create_data_table(component_data))
        story.append(Spacer(1, 20))
